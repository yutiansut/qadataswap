...
SimpleArena::SimpleArena(const std::string& name, size_t size, size_t buffer_count)
    ...
    size_t header_size = sizeof(SimpleHeader) +
                        sizeof(SimpleHeader::BufferState) * buffer_count_;
    header_size = (header_size + CACHE_LINE_SIZE - 1) & ~(CACHE_LINE_SIZE - 1); // Align

    buffer_size_ = (total_size_ - header_size) / buffer_count_;
...
bool SimpleArena::CreateSharedMemory() {
    ...
    header_ = static_cast<SimpleHeader*>(mapped_memory_);
    return true;
}

bool SimpleArena::AttachSharedMemory() {
    std::string shm_name = "/qads_" + name_;
...
void SimpleArena::InitializeHeader() {
    // Use placement new for proper initialization
    new (header_) SimpleHeader();

    // Initialize the flexible array member
    for (size_t i = 0; i < buffer_count_; ++i) {
        new (&header_->buffer_states[i]) SimpleHeader::BufferState();
    }

    header_->magic = MAGIC_NUMBER;
    header_->version = VERSION;
    header_->total_size = total_size_;
    header_->header_size = sizeof(SimpleHeader) +
                          sizeof(SimpleHeader::BufferState) * buffer_count_;
...
bool SimpleArena::WriteBytes(const uint8_t* data, size_t size) {
    if (!is_writer_ || !is_attached_) {
        return false;
    }

    if (size > buffer_size_) {
        std::cerr << "Data size exceeds buffer size\n";
        return false;
    }
    ...
    header_->buffer_states[buffer_idx].data_size.store(size);
...
bool SimpleArena::ReadBytes(uint8_t* buffer, size_t buffer_size, size_t* out_size, int timeout_ms) {
    if (is_writer_ || !is_attached_) {
        return false;
    }
    ...
    size_t buffer_offset = header_->buffers_offset + buffer_idx * buffer_size_;
    const uint8_t* src_buffer = static_cast<const uint8_t*>(mapped_memory_) + buffer_offset;
    ...
    // Copy data
    memcpy(buffer, src_buffer, data_size);
    *out_size = data_size;
...
size_t SimpleArena::GetNextWriteBuffer() {
    return header_->write_sequence.load() % buffer_count_;
}

size_t SimpleArena::GetCurrentReadBuffer() {
    return header_->read_sequence.load() % buffer_count_;
}

void SimpleArena::Close() {
    if (mapped_memory_) {
        if (is_writer_) {
